...
public class MailListener implements SendListener, ReceiveListener
{
	 ...
	 * It composes an email, attaching relevant files and mails it to a target mailbox.
	 */
	public void onReceiveEvent(ReceiveEvent event)
	{
		...
		if((event.getCidNumber() != null) && (!"".equals(event.getCidNumber())))
		{
		...
		try{
			...
			if(!f.exists())
			{
				...
				body= "A facsimile failed to be received at "+
					now+"\n\n"+
					"See the attached log file for session details.\n";
			}else{
				body= "The attached facsimile was received "+
					now+"\n";
			}
			...
			msg.addHeader("X-MailListener","$Id$");
